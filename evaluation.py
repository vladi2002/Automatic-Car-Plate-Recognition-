...
def get_args():
	...
	parser.add_argument('--file_path', type=str, default="dataset/extractedOutputTrainingsVideo.csv")
...
if __name__ == '__main__':
	...
	gt = np.unique(ground_truth['License plate'].to_list())
	st = np.unique(student_results['License plate'].to_list())

	intersection = np.intersect1d(gt, st)
	union = np.union1d(gt, st)
	iou = intersection.shape[0] / union.shape[0]
	print(iou)

	print(len(union), len(intersection), len(st), len(gt))
